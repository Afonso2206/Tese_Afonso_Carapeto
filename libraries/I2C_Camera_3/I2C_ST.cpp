...
#define SDA_HIGH  { pinMode(_sda, INPUT); sda_state=1; delayMicroseconds(DELAY_GENERIC); }
#define SDA_LOW   { if(sda_state) pinMode(_sda, OUTPUT);  sda_state=0; delayMicroseconds(DELAY_GENERIC); }
...
#define ADR7_WR(A) ((A) << 1)
#define ADR7_RD(A) (1 | (A) << 1)


// Sequences --------------------------------------------------- //
void i2c_st::begin(uint8_t sda, uint8_t scl)
{
...
void i2c_st::transmission_begin(uint8_t adr)       // Begin new transmission
{
  _adr = adr & 0xFE;        // Copy the address to a local var
...
bool i2c_st::transmission_reg_wr(uint16_t reg, uint8_t val)
{
  send_u8(_adr << 1);                  // wr: LSbit = 0
  uint8_t ret = check_ack();       // Let's check for the acknowledgement bit, was our data recieved?
  uint8_t ah, al, data;
  
  ah = (uint8_t)((reg & 0xff00) >> 8);
  al = (uint8_t)((reg & 0xff));
  ...
  send_u8(_adr << 1);                  // wr: LSbit = 0
  ret |= check_ack();       // Let's check for the acknowledgement bit, was our data recieved?
  send_u8(val);
  ret |= check_ack();
  return ret;
}
...
uint16_t i2c_st::transmission_reg_rd(uint16_t reg)
 {
  // estamos a escrever o endereço do registo
  send_u8(_adr << 1);                  // wr: LSbit = 0 - o endereço do device é deslocado 1 bit esquerda, o bit 0 é RD/WR
  uint8_t ret = check_ack();       // Let's check for the acknowledgement bit, was our data recieved?
  uint8_t ah, al;
  uint16_t data;
  ...
  // estamos a ler o conteudo do registo
  send_u8((_adr << 1) | 1);                  // rd: LSbit = 1
  ret = check_ack();       // Let's check for the acknowledgement bit, was our data recieved?
  
      data = get_u8();
      send_ack();          // We need to send an acknowledgement that we recieved the data here
      data = data << 8;
      data |= get_u8();
      check_ack();          // NAK
...
bool i2c_st::transmission_write(uint8_t *data, uint8_t len)     // Write byte(s) to I2C - called after begin
{
...
bool i2c_st::transmission_read(uint8_t *data, uint8_t len)   // Read byte(s) from I2C - called after begin
{
...
void i2c_st::transmission_end()          // End transmission 
{
...
uint8_t i2c_st::get_u8()          // Get (e.g. read) a byte over I2C
{
  ...
  { 
    ...
    delayMicroseconds(DELAY_GENERIC);
    scl_tick();
    delayMicroseconds(DELAY_GENERIC);
  }  
...
void i2c_st::send_u8(uint8_t data)       // Send a byte over I2C
{
...
bool i2c_st::check_ack()                  // Check for the akcnowledgment bit
{
...
void i2c_st::send_ack()     // send acknowledgement bit
{
...
void i2c_st::scl_tick()                     // SCL clock, i.e. signal that data is ready/required
{
...
void i2c_st::stop()                 // Stop sequence - we have finished sending/recieving data
{
...
void i2c_st::start()              // Start sequence - start trading bits
{
